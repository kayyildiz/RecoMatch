...
# ==========================================
# 3. YARDIMCI FONKSİYONLAR & PARSER
# ==========================================
...
def parse_amount(val):
    """
    GÜÇLENDİRİLMİŞ SAYI PARSER:
    - Zaten sayı ise (int/float) -> Doğrudan döndür (HATA ÇÖZÜMÜ)
    - Metin ise (1.000,50 veya 1,000.50) -> Temizle ve çevir.
    """
    if pd.isna(val) or val == "":
        return 0.0
    
    # 1. Zaten sayısal mı? (Excel'den direkt okunanlar)
    ...
    # 2. Metin Temizliği
    # Olası: "1.234,56" (TR) veya "1,234.56" (US) veya "1234.56"
    try:
        if "," in s and "." in s:
            if s.rfind(",") > s.rfind("."): # TR Format: 1.234,56
                s = s.replace(".", "").replace(",", ".")
            else: # US Format: 1,234.56
                s = s.replace(",", "")
        elif "," in s: # Sadece virgül (12,50) -> 12.50 yap
            s = s.replace(",", ".")
...
def calculate_smart_balance(row, role, 
    ...
    if mode_fx == "separate":
        ...
        if raw_fx != 0:
            # Yönü TL'den al (Akıllı Yön Tespiti)
            if mode_tl == "separate":
                if tl_debt_val > 0 and tl_credit_val == 0: fx_net = -abs(raw_fx) # Borç
                elif tl_credit_val > 0 and tl_debt_val == 0: fx_net = abs(raw_fx) # Alacak
                else: fx_net = raw_fx * sign # Fallback (TL tek kolon mantığı)
            else:
                fx_net = raw_fx * sign
